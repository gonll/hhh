...
// Crear tabla noremunerativo si no existe
$res_check = mysqli_query($conexion, "SHOW TABLES LIKE 'noremunerativo'");
if (!$res_check || mysqli_num_rows($res_check) == 0) {
    $sql_create = "CREATE TABLE IF NOT EXISTS noremunerativo (
    ...
    if (isset($_POST['guardar'])) {
        ...
        if ($descripcion === '') {
            ...
            if (isset($_POST['id']) && (int)$_POST['id'] > 0) {
                $id = (int)$_POST['id'];
                $sql = "UPDATE noremunerativo SET descripcion = '$descripcion', monto = $monto WHERE id = $id";
                if (mysqli_query($conexion, $sql)) {
                    $mensaje = 'Registro actualizado.';
                    header('Location: abm_noremunerativo.php?ok=1');
                    exit;
                } else {
                    $mensaje = 'Error: ' . mysqli_error($conexion);
                }
            } else {
                $sql = "INSERT INTO noremunerativo (descripcion, monto) VALUES ('$descripcion', $monto)";
                if (mysqli_query($conexion, $sql)) {
        ...
        $id = (int)$_POST['id'];
        if ($id > 0 && mysqli_query($conexion, "DELETE FROM noremunerativo WHERE id = $id")) {
...
if (isset($_GET['editar'])) {
    ...
    if ($editar_id > 0) {
        $r = mysqli_query($conexion, "SELECT * FROM noremunerativo WHERE id = $editar_id LIMIT 1");
...
$lista = [];
$res_lista = mysqli_query($conexion, "SELECT * FROM noremunerativo ORDER BY descripcion ASC");
if ($res_lista) {
...
<head>
    ...
    <title>ABM No Remunerativo - HHH</title>
    <style>
        ...
        h2 { color: #17a2b8; margin-top: 0; margin-bottom: 15px; font-size: 18px; border-bottom: 2px solid #17a2b8; padding-bottom: 8px; }
    ...
    <div class="container">
        ...
        <form method="POST" class="form-nav-enter">
            ...
            <div class="form-row">
                <div class="campo-ancho">
                    ...
                    <input type="text" name="descripcion" maxlength="255" required
                        ...
                        placeholder="Ej: Viático, Bonificación, etc.">
                </div>
                <div class="campo">
                    <label>Monto ($)</label>
            ...
            <div class="form-row">
                ...
                <?php if ($editar_id): ?>
                    <a href="abm_noremunerativo.php" class="btn btn-secondary">Cancelar</a>
                <?php endif; ?>
            </div>
        </form>
            ...
            <tbody>
                <?php if (count($lista) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="acciones">
                                <a href="abm_noremunerativo.php?editar=<?= (int)$r['id'] ?>" class="btn btn-secondary" style="padding: 4px 10px; font-size: 11px;">Modificar</a>
