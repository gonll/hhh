<?php
/**
 * Libro global de movimientos por transferencia (no registrados en Caja efectivo).
 * Comprobantes: TRANSFERENCIA, ALQUILER TRANSF, EXP/TRANSF, NIVELACION TRANSF
 *
 * EXP/TRANSF por cobro de expensa: se graba también en consorcio y propietario (espejos).
 * En este libro solo debe verse una vez: la fila del pagador (concepto EXPENSAS … o PAGO EXPENSAS…).
 */
include 'db.php';
...
// Excluir EXP/TRANSF con concepto "COBRO EXPENSA…" (consorcio / propietario); mantener línea del inquilino/pagador.
$filtroExpTransfSoloPagador = "(UPPER(TRIM(c.comprobante)) <> 'EXP/TRANSF' OR UPPER(c.concepto) LIKE 'EXPENSAS %' OR UPPER(c.concepto) LIKE 'PAGO EXPENSAS%' OR UPPER(c.concepto) LIKE 'PAGO DE EXPENSAS%')";
$filtroLibro = "$filtroCompro AND $filtroAmbito AND $filtroExpTransfSoloPagador";
...
$res_total = mysqli_query($conexion, "SELECT COALESCE(SUM(c.monto), 0) AS total FROM cuentas c INNER JOIN usuarios u ON u.id = c.usuario_id WHERE $filtroLibro");
...
if (count($filas) > 0) {
    ...
    $r_older = mysqli_query($conexion, "SELECT 1 FROM cuentas c INNER JOIN usuarios u ON u.id = c.usuario_id WHERE $filtroLibro AND (c.fecha < '$bf_esc' OR (c.fecha = '$bf_esc' AND c.movimiento_id < $first_id)) LIMIT 1");
    $has_more_older = ($r_older && mysqli_num_rows($r_older) > 0);

    $lf_esc = mysqli_real_escape_string($conexion, $last_fecha);
    $r_newer = mysqli_query($conexion, "SELECT 1 FROM cuentas c INNER JOIN usuarios u ON u.id = c.usuario_id WHERE $filtroLibro AND (c.fecha > '$lf_esc' OR (c.fecha = '$lf_esc' AND c.movimiento_id > $last_id)) LIMIT 1");
...
if (count($filas) > 0) {
    ...
    $r_sum = mysqli_query($conexion, "SELECT COALESCE(SUM(c.monto), 0) AS s FROM cuentas c INNER JOIN usuarios u ON u.id = c.usuario_id WHERE $filtroLibro AND (c.fecha > '$pf_esc' OR (c.fecha = '$pf_esc' AND c.movimiento_id > $pid))");
...
function renderFilaTransf($m, &$saldo, $conexion) {
    $saldo += $m['monto'];
    ...
    $celdaEliminar = $mostrarX
        ...
        : "";
    $cuenta = isset($m['nom_cuenta']) ? strtoupper(trim($m['nom_cuenta'])) : '';
    $concepto_show = $cuenta !== '' ? ($cuenta . ' · ' . strtoupper($m['concepto'])) : strtoupper($m['concepto']);
    ...
    $concepto_show_esc = htmlspecialchars($concepto_show, ENT_QUOTES, 'UTF-8');
    return "<tr class='fila-mov'$tr_style data-movimiento-id=\"{$m['movimiento_id']}\" data-concepto=\"$concepto_attr\" data-compro=\"$compro_attr\" data-ref=\"$ref_attr\" data-monto=\"" . (float)$m['monto'] . "\" data-arriendo-id=\"$arriendo_id\" data-arriendo-fecha=\"$arriendo_fecha\" data-fecha=\"$fecha_raw\" onclick=\"seleccionarFila(this, '{$m['movimiento_id']}', '$fechaFormateada', '" . addslashes(strtoupper($m['concepto'])) . "', '" . addslashes(strtoupper($m['comprobante'])) . "', '" . addslashes(strtoupper($m['referencia'])) . "', '{$m['monto']}')\">
        ...
        <td class='al-izq'>$concepto_show_esc</td>
...
if (count($filas) > 0) {
    foreach ($filas as $m) {
        $html .= renderFilaTransf($m, $saldo, $conexion);
    }
} else {
    $html = "<tr><td colspan='7' style='text-align:center; padding:30px; color:gray;'>NO HAY MOVIMIENTOS POR TRANSFERENCIAS</td></tr>";
}
