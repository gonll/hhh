...
<head>
    ...
    <style>
        ...
        .col-propiedad { width: 22%; }
        .col-consorcio { width: 18%; }
        .col-padron { width: 10%; }
        .col-detalle { width: 15%; }
        .col-estado { width: 13%; }
        .col-inicio { width: 10%; }
        .col-vencimiento { width: 10%; }
        ...
        .btn-volver { background: #6c757d; color: white; margin-top: 15px; font-size: 10px; padding: 5px 15px; text-decoration: none; }
...
<div class="contenedor">
    ...
    <div class="controles">
        ...
        <div id="barraEditar" style="display:none; margin-top:8px;">
            <a href="#" id="linkEditarProp" class="btn btn-editar w-grande" style="text-decoration:none; padding:6px 12px;">✏️ EDITAR DATOS</a>
        </div>
    </div>
                ...
                <td>
                    <div class="flex-acciones" onclick="event.stopPropagation()">
                        <?php if($alquilada): ?>
                            ...
                            <button onclick="imprimirContrato(<?= $f['propiedad_id'] ?>)" class="btn btn-print w-chico">PRINT</button>
                        <?php else: ?>
...
function seleccionarPropiedad(id, filaEl, ev) {
    ...
    var link = document.getElementById('linkEditarProp');
    if (barra && link) {
        link.href = 'editar_propiedad.php?id=' + id;
...
function eliminarPropiedad(id) {
    if(confirm("¿Eliminar propiedad definitivamente?")) {
        fetch('eliminar_propiedad.php?id='+id).then(r=>r.text()).then(res=>{
