...
// Crear tabla asignaciones si no existe
$res_check = mysqli_query($conexion, "SHOW TABLES LIKE 'asignaciones'");
if (!$res_check || mysqli_num_rows($res_check) == 0) {
    $sql_create = "CREATE TABLE IF NOT EXISTS asignaciones (
    ...
    if (isset($_POST['guardar'])) {
        ...
        if ($descripcion === '') {
            ...
            if (isset($_POST['id']) && (int)$_POST['id'] > 0) {
                $id = (int)$_POST['id'];
                $sql = "UPDATE asignaciones SET descripcion = '$descripcion', monto = $monto WHERE id = $id";
                if (mysqli_query($conexion, $sql)) {
                    $mensaje = 'Registro actualizado.';
                    header('Location: abm_asignaciones.php?ok=1');
                    exit;
                } else {
                    $mensaje = 'Error: ' . mysqli_error($conexion);
                }
            } else {
                $sql = "INSERT INTO asignaciones (descripcion, monto) VALUES ('$descripcion', $monto)";
                if (mysqli_query($conexion, $sql)) {
        ...
        $id = (int)$_POST['id'];
        if ($id > 0 && mysqli_query($conexion, "DELETE FROM asignaciones WHERE id = $id")) {
...
if (isset($_GET['editar'])) {
    ...
    if ($editar_id > 0) {
        $r = mysqli_query($conexion, "SELECT * FROM asignaciones WHERE id = $editar_id LIMIT 1");
...
$lista = [];
$res_lista = mysqli_query($conexion, "SELECT * FROM asignaciones ORDER BY descripcion ASC");
if ($res_lista) {
...
<head>
    ...
    <title>ABM Asignaciones - HHH</title>
    <style>
    ...
    <div class="container">
        ...
        <form method="POST" class="form-nav-enter">
            ...
            <div class="form-row">
                <div class="campo-ancho">
                    ...
                    <input type="text" name="descripcion" maxlength="255" required
                        ...
                        placeholder="Ej: Antigüedad, Presentismo, etc.">
                </div>
                <div class="campo">
                    <label>Monto ($)</label>
            ...
            <div class="form-row">
                ...
                <?php if ($editar_id): ?>
                    <a href="abm_asignaciones.php" class="btn btn-secondary">Cancelar</a>
                <?php endif; ?>
            </div>
        </form>
            ...
            <tbody>
                <?php if (count($lista) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="acciones">
                                <a href="abm_asignaciones.php?editar=<?= (int)$r['id'] ?>" class="btn btn-secondary" style="padding: 4px 10px; font-size: 11px;">Modificar</a>
